...
module vtkxml
    ...
    interface vtkxml_write_slice
        !module procedure parallel_vect3D, vtkxml_scalar
        module procedure vtkxml_scalar_slice
    end interface vtkxml_write_slice
    ...
    type io_data
        ...
        integer                                 :: iteration
        !> indice of the current output for 2D slice - in order to construct time sequence.
        integer                                 :: iteration_slice
        !> mesh information
    ...
    ! Specifique procedure to write slice of 3D field in distribued vtk files
    public                  :: vtkxml_scalar_slice
   ...
   ! character (len=50), private                         :: dir = './save/'
    ...
    real(WP), parameter, private                        :: minValue = 1e-9
...
subroutine vtkxml_init_field_basic(f_name, tag, mesh)
    ...
    field_data(tag)%iteration_slice  = 0
...
subroutine vtkxml_scalar(tag, values, f_name)
    ...
    write(44, '(4x,a,f7.4,x,f7.4,x,f7.4,a)') 'Spacing="', field_data(tag)%mesh%dx(1), &
...
subroutine parallel_file_description(tag)
    ...
    write(45, '(4x,a,f8.5,x,f8.5,x,f8.5,a)') 'Spacing="', field_data(tag)%mesh%dx(1), &
        & field_data(tag)%mesh%dx(2), field_data(tag)%mesh%dx(3), '">'
...
!> Write an output containing a 2D slice of an "one-component" field (eg scalar)
!!    @param[in,out]    tag     = tag associated to the field to save (ie matching indice in the table "field_data")
!!    @param[in]        values  = field values - the values to write in the file
!!    @param[in]        f_name  = name of the field (optional, redondant with tag, can be used to check it)
!!    @param[in]        Y_coord = Y coordinate of the cut (optional, by default the middle)
!!    @param[in]        Z_coord = Z coordinate of the cut (optional, by default the middle)
subroutine vtkxml_scalar_slice(tag, values, f_name, Y_coord, Z_coord)

    integer, intent(inout)                              :: tag
    character (len=*), intent(in), optional             :: f_name
    real(WP), dimension(:,:,:), intent(in)              :: values
    real(WP), intent(in), optional                      :: Y_coord, Z_coord
    
    character (len=long_name+size_rank+5)               :: file_name    ! output name
    integer                                             :: k            ! some array indices
    integer                                             :: i_slice      ! X-indice for the cut.
    character (len=short_name)                          :: f_name_bis
    ...
    ! Write the parallel file wich contain information about distribution (but no datas)
    if (rank3D==0) call parallel_file_description_slice(tag)

    ! Write local file
    ! Open file
    file_name = trim(dir)//'slice'//trim(output_name_slice(tag, rank3D))
    if (binary) then
        !open(unit = 44, file=file_name, status='new', access='stream')
        open(unit = 44, file=file_name, form='formatted')
    else
    ...
    if (binary) then
        ...
        write(44,'(8x,a,a,a)') '<DataArray type="Float64" Name="', &
                & trim(field_data(tag)%f_name),'" NumberOfComponents="1" format="ascii">'
        ! Data in ascii format
        write(format_data,'(a,i0,a)') '(',field_data(tag)%mesh%N_proc(2), '(f15.5,x))'

        i_slice = (size(values,1)/2)
        do k = 1, field_data(tag)%mesh%N_proc(3)
            write(44, format_data) values(i_slice,:,k)
        end do
    end if
    ...
    close(44)
    field_data(tag)%iteration_slice = field_data(tag)%iteration_slice + 1

end subroutine vtkxml_scalar_slice


!> Write the parallel file wich contain information about distribution (but no datas) for parallel vtk output.
!!    @param[in,out]    tag     = tag associated to the field to save (ie matching indice in the table "field_data")
subroutine parallel_file_description_slice(tag)

    integer, intent(in)         :: tag

    character (len=long_name+5) :: file_name    ! output name
    integer                     :: rank

    ! ===== Write data ===== 

    ! Open file
    file_name = trim(dir)//'slice'//trim(output_name_slice(tag))
    open(unit = 45, file=file_name, form='formatted')
    ...
    write(45, '(a)') '<VTKFile type="PImageData" version="0.1" byte_order="LittleEndian">'
    write(45,'(2x,a,i1,x,i0,x,i1,x,i0,x,i1,x,i0,a)') '<PImageData WholeExtent="', &
            & 1, 1, 1, field_data(tag)%mesh%N(2), 1, field_data(tag)%mesh%N(3), '"'
    write(45, '(4x,a)') 'GhostLevel="0"'
    write(45, '(4x,a)') 'Origin="0 0 0"'
    write(45, '(4x,a,f8.5,x,f8.5,x,f8.5,a)') 'Spacing="', field_data(tag)%mesh%dx(1), &
        & field_data(tag)%mesh%dx(2), field_data(tag)%mesh%dx(3), '">'
    ...
    do rank = 0, total_nb_proc-1
        write(45, '(4x,a,i0,x,i0,x,i0,x,i0,x,i0,x,i0,a)') '<Piece Extent="', &
            ...
            & field_data(tag)%piece_extend(rank,3,1), &
            & field_data(tag)%piece_extend(rank,3,2),'"'
        write(45, '(6x,a8,a,a)') 'Source="','slice'//trim(output_name_slice(tag,rank)),'"/>'
    end do
...
end subroutine parallel_file_description_slice
...
!!    @param[in]    rank    = rank of the current processus
!!    @return       io_name = name of the output
function output_name_slice(tag, rank) result(io_name)
    ...
    write(io_name,format_no_rank) trim(field_data(tag)%f_name), &
        & '-',field_data(tag)%iteration_slice,'.pvti'
    if (present(rank)) then
        write(io_name,format_rank) trim(field_data(tag)%f_name), &
            &'_p',rank,'-',field_data(tag)%iteration_slice,'.vti'
    end if

end function output_name_slice
