...
IMPLICIT NONE
    ...
    END TYPE REAL_DATA_LAYOUT


    INTERFACE ASSIGNMENT(=)
        MODULE PROCEDURE real_affectation
    END INTERFACE
...
!------------------------------------------------------------------------------
LOGICAL FUNCTION real_initDataLayout(name,val,nx,ny,nz,Lx,Ly,Lz,nbcpus,me,how)
    IMPLICIT NONE
    ...
    ELSE
        val%values=0.0_WP
        real_initDataLayout=.TRUE.
    ENDIF
        RETURN

END FUNCTION real_initDataLayout
...
!------------------------------------------------------------------------------
    SUBROUTINE real_deleteDataLayout(val)
    TYPE(REAL_DATA_LAYOUT),INTENT(INOUT) :: val
    IF(ASSOCIATED(val%values))  DEALLOCATE(val%values)
    IF(ASSOCIATED(val%Y4layoutOnX))  DEALLOCATE(val%Y4layoutOnX)
    IF(ASSOCIATED(val%Z4layoutOnX))  DEALLOCATE(val%Z4layoutOnX)
    ...
    val%zmin=0
    val%zmax=-1
    END SUBROUTINE real_deleteDataLayout
...
!> @param[in] me my rank in the cpus pool. Range in  [0,nbcpus-1]
!------------------------------------------------------------------------------
    SUBROUTINE real_showlayout(val,me)
...
101 FORMAT(a," on cpu ",i0," is [",i3,":",i3,"]")
    ...
    END SUBROUTINE real_showlayout
...
!
!> \brief
!> Check that the different REAL_DATA_LAYOUT argument have the same layout :
!> same global dimensions and same local dimensions.
!> @param[in] val is the variable of type REAL_DATA_LAYOUT for wich the layout is checked
!> @param[in] val1 is a variable of type REAL_DATA_LAYOUT to compare with
!> @param[in] val2 is an optional variable of type REAL_DATA_LAYOUT to compare with
!> @param[in] val3 is an optional variable of type REAL_DATA_LAYOUT to compare with
!> @param[in] val4 is an optional variable of type REAL_DATA_LAYOUT to compare with
!> @param[in] val5 is an optional variable of type REAL_DATA_LAYOUT to compare with
!> @return .TRUE. if all the provided variable have the same layout
!------------------------------------------------------------------------------
    RECURSIVE FUNCTION real_samelayout(val,val1,val2,val3,val4,val5) result(res)

    IMPLICIT NONE

    TYPE(REAL_DATA_LAYOUT),INTENT(IN) :: val, val1
    TYPE(REAL_DATA_LAYOUT),INTENT(IN), OPTIONAL :: val2
    TYPE(REAL_DATA_LAYOUT),INTENT(IN), OPTIONAL :: val3
    TYPE(REAL_DATA_LAYOUT),INTENT(IN), OPTIONAL :: val4
    TYPE(REAL_DATA_LAYOUT),INTENT(IN), OPTIONAL :: val5

    LOGICAL :: res

    IF (PRESENT(val5)) THEN
      res=(real_samelayout(val,val1) &
         &.AND. &
	 & real_samelayout(val1,val2)&
         &.AND. &
	 & real_samelayout(val2,val3)&
         &.AND. &
	 & real_samelayout(val3,val4)&
         &.AND. &
	 & real_samelayout(val4,val5)&
	 &)
...
!
!> \brief
!> Support for the = operator between two type REAL_DATA_LAYOUT:
!> same global dimensions and same local dimensions.
!> @param [in,out] left is the variable of type REAL_DATA_LAYOUT to set
!> @param [in] right is the variable of type REAL_DATA_LAYOUT to copy in left
!------------------------------------------------------------------------------
    SUBROUTINE real_affectation(left, right)

       IMPLICIT NONE
       TYPE (REAL_DATA_LAYOUT), INTENT(INOUT) :: left
       TYPE (REAL_DATA_LAYOUT), INTENT(IN) :: right
       ...
       left%ly         =right%ly
       left%lz         =right%lz
    END SUBROUTINE real_affectation
...
!
!> \brief
!> Support for the copy of the structure of type REAL_DATA_LAYOUT without
!> value initialisation (values are set to 0.0_WP). But all the architecture
!> and parallel informaytion is duplicated.
!> @param[in] ori is the variable model of type REAL_DATA_LAYOUT
!> @param[in,out] copy is the variable of type REAL_DATA_LAYOUT builded
!> @param[in] nameOut is the name of the build variable instead of the name of the ori (optional)
!------------------------------------------------------------------------------
    FUNCTION real_copyStructOnly(ori, copy, nameOut) result(res)

       IMPLICIT NONE
       TYPE (REAL_DATA_LAYOUT), INTENT(INOUT) :: copy
       TYPE (REAL_DATA_LAYOUT), INTENT(IN) :: ori
       ...
       copy%lz         =ori%lz
       res=.TRUE.
    END FUNCTION real_copyStructOnly
...
!! @param[in,out] first   is the first variable of type REAL_DATA_LAYOUT
!! @param[in,out] second  is the second variable of type REAL_DATA_LAYOUT
!! @param[in,out] third   is the second variable of type REAL_DATA_LAYOUT
!! @return        res     = true if everything is ok.
!------------------------------------------------------------------------------
FUNCTION real_permute(first,second,third) result(res)

  IMPLICIT NONE
  ! Input/Output
  TYPE (REAL_DATA_LAYOUT), INTENT(INOUT) :: first, second
  TYPE (REAL_DATA_LAYOUT), INTENT(INOUT), optional :: third
  LOGICAL :: res
  ! Local
  ...
  if(present(third)) then
    if(.not. real_samelayout(first,second, third)) then
      write(6,'(a)')'[ERROR] permute for TYPE (REAL_DATA_LAYOUT) : not same layout!'
      return
    end if
    ...
    if(.not. real_samelayout(first,second)) then
      write(6,'(a)')'[ERROR] permute for TYPE (REAL_DATA_LAYOUT) : not same layout!'
      return
    end if
...
!------------------------------------------------------------------------------
FUNCTION real_foundProcess(i,j,k,layout) result(proc_rank)
...
END FUNCTION real_foundProcess


END MODULE realdatalayout
!> @}
