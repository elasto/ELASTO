...
!------------------------------------------------------------------------------
!> Surcharge of cmplx_setAlongX for vector.
!> @author 
!> Patrick BEGOU
!
!> @details
...
FUNCTION cmplx_setAlongX_Vector(spec_rank,val)
    ...
    LOGICAL                                     :: cmplx_setAlongX_Vector

    TYPE(COMPLEX_DATA_LAYOUT)  :: comp_data
    INTEGER                 :: comp
    

    cmplx_setAlongX_Vector=.FALSE.
    ...
    CASE (alongX)  
        cmplx_setAlongX_Vector=.TRUE.
        RETURN
    CASE (alongY)
        do comp = 1, val%nb_components
            call cmplx_vector_manipulate_comp(val,comp, comp_data, .true.)
            if(.not. cmplx_AlongYtoAlongX(spec_rank,comp_data)) RETURN
        end do
    CASE (alongZ)
        do comp = 1, val%nb_components
            call cmplx_vector_manipulate_comp(val,comp, comp_data, .true.)
            if(.not. cmplx_AlongZtoAlongY(spec_rank,comp_data)) RETURN
            if(.not. cmplx_AlongYtoAlongX(spec_rank,comp_data)) RETURN
        end do
    END SELECT
...
END FUNCTION cmplx_setAlongX_Vector
...
!> @param[in] spec_rank my rank in spec_communitor
!------------------------------------------------------------------------------
FUNCTION cmplx_setAlongY_Vector(spec_rank,val)
    USE datalayout
    USE communicators_tools
    USE cmplxVectordatalayout
    ...
    TYPE(COMPLEX_VECTOR_DATA_LAYOUT),INTENT(INOUT) :: val
    INTEGER, INTENT(IN)                         :: spec_rank
    LOGICAL                                     :: cmplx_setAlongY_Vector
    
    TYPE(COMPLEX_DATA_LAYOUT)  :: comp_data
    INTEGER                 :: comp

    cmplx_setAlongY_Vector=.FALSE.
    ...
    CASE (alongX)  
        do comp = 1, val%nb_components
            call cmplx_vector_manipulate_comp(val,comp, comp_data, .true.)
            if(.not. cmplx_AlongXtoAlongY(spec_rank,comp_data)) RETURN
        end do
    CASE (alongY)
        cmplx_setAlongY_Vector=.TRUE.
        RETURN
    CASE (alongZ)
        do comp = 1, val%nb_components
            call cmplx_vector_manipulate_comp(val,comp, comp_data, .true.)
...
END FUNCTION cmplx_setAlongY_Vector
...
!> @param[in] spec_rank my rank in spec_communitor
!------------------------------------------------------------------------------
FUNCTION cmplx_setAlongZ_Vector(spec_rank,val)
    USE datalayout
    USE communicators_tools
    USE cmplxVectordatalayout
    ...
    TYPE(COMPLEX_VECTOR_DATA_LAYOUT),INTENT(INOUT) :: val
    INTEGER, INTENT(IN)             :: spec_rank
    LOGICAL                         :: cmplx_setAlongZ_Vector

    TYPE(COMPLEX_DATA_LAYOUT)  :: comp_data
    INTEGER                 :: comp

    cmplx_setAlongZ_Vector=.FALSE.
    ...
    CASE (alongX)  
        do comp = 1, val%nb_components
            call cmplx_vector_manipulate_comp(val,comp, comp_data, guru=.true.)
            if(.not. cmplx_AlongXtoAlongY(spec_rank,comp_data)) RETURN
            if(.not. cmplx_AlongYtoAlongZ(spec_rank,comp_data)) RETURN
        end do
    CASE (alongY)
        do comp = 1, val%nb_components
            call cmplx_vector_manipulate_comp(val,comp, comp_data, guru=.true.)
...
END FUNCTION cmplx_setAlongZ_Vector
...
!!
!!> @details
!!> This function dump in a binary file the values of the COMPLEX_DATA_LAYOUT 
...
!!> @param[in] val the COMPLEX_DATA_LAYOUT variable to save in the file.
!!> @return .TRUE. if file is successfuly written.
!!------------------------------------------------------------------------------
!FUNCTION cmplx_WriteToFile(spec_rank,name,val)
!!
...
!  offset(2) = val%ymin - 1
!  offset(3) = val%zmin - 1
!  CALL MPI_TYPE_CREATE_SUBARRAY(3,gsizes,lsizes,offset,MPI_ORDER_FORTRAN,MPI_COMPLEX_WP,fileview,ierr)
!  IF (ierr .NE. MPI_SUCCESS) THEN
!    WRITE(6,'(a,i0,a)')"ERROR cmplx_WriteToFile on process ",spec_rank,": unable to create subarray!"
!    RETURN
!  ENDIF
!  
!  CALL MPI_TYPE_COMMIT(fileview,ierr)
!  IF (ierr .NE. MPI_SUCCESS) THEN
!    WRITE(6,'(a,i0,a)')"ERROR cmplx_WriteToFile on process ",spec_rank,": subarray type commit fails!"
!    RETURN
!  ENDIF
!  
...
!  IF (ierr .NE. MPI_SUCCESS) THEN
!    WRITE(6,'(a,i0,a)')"ERROR cmplx_WriteToFile on process ",spec_rank,": cannot open file ["//trim(name)//"]"
!    RETURN
!  ENDIF
!  
...
!     IF (ierr .NE. MPI_SUCCESS) THEN
!       WRITE(6,'(a,i0,a)')"ERROR cmplx_WriteToFile on process ",spec_rank,&
...
!
!  ! Positionnement
!  CALL MPI_FILE_SET_VIEW(iunit,disp,MPI_COMPLEX_WP,fileview,"native",MPI_INFO_NULL,ierr)
!  IF (ierr .NE. MPI_SUCCESS) THEN
!    WRITE(6,'(a,i0,a)')"ERROR cmplx_WriteToFile on process ",spec_rank,&
...
!  
!  ! Write data
!  CALL MPI_FILE_WRITE_ALL(iunit,val%values,lsizes(1)*lsizes(2)*lsizes(3),MPI_COMPLEX_WP,status,ierr)
!  IF (ierr .NE. MPI_SUCCESS) THEN
!    WRITE(6,'(a,i0,a)')"ERROR cmplx_WriteToFile on process ",spec_rank,&
...
!
!  !All is OK now!
!  cmplx_WriteToFile=.TRUE.
!  RETURN
!
!END FUNCTION cmplx_WriteToFile
!
!!------------------------------------------------------------------------------
!!> @author 
!!> Antoine Vollant
...
END MODULE cmplxVectorparallel
