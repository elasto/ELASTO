...
module cmplxVectordatalayout
    ...
    type COMPLEX_VECTOR_DATA_LAYOUT
        ! -- Generic information --
    ...
    end type COMPLEX_VECTOR_DATA_LAYOUT
    
    
    interface assignment(=)
        module procedure cmplx_vector_affectation
    end interface
...
!    private initlayoutarray
    private cmplx_vector_affectation
...
!------------------------------------------------------------------------------
logical function cmplx_vector_initdatalayout(name,names_comp,val,nb_components,nx,ny,nz,lx,ly,lz,nbcpus,rank,how)
    ...
    type(COMPLEX_VECTOR_DATA_LAYOUT),intent(inout) :: val
    ...
    ! -- Init --
    cmplx_vector_initdatalayout=.false.
    ...
    if (ires .ne.0) then
        ...
        val%values=0.0_wp
        cmplx_vector_initdatalayout=.true.
    endif

    return

end function cmplx_vector_initdatalayout
...
!
!
!> @param[in,out] val is the variable of type COMPLEX_VECTOR_DATA_LAYOUT to be freed
!------------------------------------------------------------------------------
subroutine cmplx_vector_deletedatalayout(val)
...
end subroutine cmplx_vector_deletedatalayout    
...
!------------------------------------------------------------------------------
subroutine cmplx_vector_showlayout(val,me)

    implicit none
    integer, intent(in)::me
    type(COMPLEX_VECTOR_DATA_LAYOUT),intent(in) :: val
    integer ::i
    write(me+10,'(a)')"===================================================================="
    write(me+10,'(a)')'i am a cmplx data layout object called "'//trim(val%name)//'"'
    write(me+10,'(a)')"===================================================================="
...
end subroutine cmplx_vector_showlayout
...
!
!> \brief
!> check that the different COMPLEX_VECTOR_DATA_LAYOUT argument have the same layout :
!> same global dimensions and same local dimensions.
!> @param[in] val is the variable of type COMPLEX_VECTOR_DATA_LAYOUT for wich the layout is checked
!> @param[in] val1 is a variable of type COMPLEX_VECTOR_DATA_LAYOUT to compare with
!> @param[in] val2 is an optional variable of type COMPLEX_VECTOR_DATA_LAYOUT to compare with
!> @param[in] val3 is an optional variable of type COMPLEX_VECTOR_DATA_LAYOUT to compare with
!> @param[in] val4 is an optional variable of type COMPLEX_VECTOR_DATA_LAYOUT to compare with
!> @param[in] val5 is an optional variable of type COMPLEX_VECTOR_DATA_LAYOUT to compare with
!> @return .true. if all the provided variable have the same layout
!------------------------------------------------------------------------------
recursive function cmplx_vector_samelayout(val,val1,val2,val3,val4,val5) result(res)

    implicit none
    
    type(COMPLEX_VECTOR_DATA_LAYOUT),intent(in) :: val, val1
    type(COMPLEX_VECTOR_DATA_LAYOUT),intent(in), optional :: val2
    type(COMPLEX_VECTOR_DATA_LAYOUT),intent(in), optional :: val3
    type(COMPLEX_VECTOR_DATA_LAYOUT),intent(in), optional :: val4
    type(COMPLEX_VECTOR_DATA_LAYOUT),intent(in), optional :: val5
    
    logical :: res
    
    if (present(val5)) then
        res=(cmplx_vector_samelayout(val,val1) &
            &.and. &
        & cmplx_vector_samelayout(val1,val2)&
            &.and. &
        & cmplx_vector_samelayout(val2,val3)&
            &.and. &
        & cmplx_vector_samelayout(val3,val4)&
            &.and. &
        & cmplx_vector_samelayout(val4,val5))
...
end function cmplx_vector_samelayout
...
!
!> \brief
!> support for the = operator between two type COMPLEX_VECTOR_DATA_LAYOUT:
!> same global dimensions and same local dimensions.
!> @param [in,out] left is the variable of type COMPLEX_VECTOR_DATA_LAYOUT to set
!> @param [in] right is the variable of type COMPLEX_VECTOR_DATA_LAYOUT to copy in left
!------------------------------------------------------------------------------
    subroutine cmplx_vector_affectation(left, right)
    
        type (COMPLEX_VECTOR_DATA_LAYOUT), intent(inout) :: left
        type (COMPLEX_VECTOR_DATA_LAYOUT), intent(in) :: right
        ...
        if (.not. associated(right%values)) then
          write(6,'(a,i0,a)')'[error] operator = for type (cmplx_data_layout) : receive uninitialized data!'
        ...
        if (ires .ne.0) then
          write(6,'(a,i0,a)')'[error] operator = for type (cmplx_data_layout) : not enought memory!'
          call mpi_finalize(ires)
          stop 'failed'
        endif 
        values=right%values 
     ...
     end subroutine cmplx_vector_affectation
...
!
!> \brief
!> support for the copy of the structure of type COMPLEX_VECTOR_DATA_LAYOUT without
!> value initialisation (values are set to 0.0_wp). but all the architecture
!> and parallel information is duplicated.
!> @param[in] ori is the variable model of type COMPLEX_VECTOR_DATA_LAYOUT 
!> @param[in,out] copy is the variable of type COMPLEX_VECTOR_DATA_LAYOUT builded
...
!------------------------------------------------------------------------------
    function cmplx_vector_copyStructOnly(ori, copy, name_out, names_compout) result(res)
    
        type (COMPLEX_VECTOR_DATA_LAYOUT), intent(inout) :: copy
        type (COMPLEX_VECTOR_DATA_LAYOUT), intent(in) :: ori
        ...
        if (.not. associated(ori%values)) then
          write(6,'(a,i0,a)')'[error]copystructonly for type (cmplx_data_layout) : receive uninitialized data!'
          return
        endif  
        ...
        if (ires .ne.0) then
            write(6,'(a,i0,a)')'[error] copystructonly for type (cmplx_data_layout) : not enought memory!'
            return
        endif 
        copy%values=0.0_wp 
    ...
    end function cmplx_vector_copystructonly


!------------------------------------------------------------------------------
!> Manipulate DIRECTLY the value of one component at the COMPLEX_DATA_LAYOUT format.
!! @author
!! Jean-Baptiste Lagaert
!
!> @details
!!     The returned values is COMPLEX_DATA_LAYOUT whose each of is pointer component
...
!! copy (for instance copy=cmplx_vector_manipulate component) by using the affection
...
!! component still point to part of the vector, delete the COMPLEX_DATA_LAYOUT mean delete
!! part of the COMPLEX_VECTOR_DATA_LAYOUT too.
...
!! @param[in]       vector      = COMPLEX_VECTOR_DATA_LAYOUT that you want manipulate.
!! @param[in]       comp        = indice of the component you want
!! @param[in,out]   comp_data   = COMPLEX_DATA_LAYOUT whose datas are pointing to the chosen
...
!> @param[in] names_out is the names of the build variable instead of the names of the ori (optional)
!------------------------------------------------------------------------------
    subroutine cmplx_vector_manipulate_comp(vector, comp, comp_data, guru)

    use cmplxdatalayout

        type (COMPLEX_VECTOR_DATA_LAYOUT), intent(in)  :: vector
        type (COMPLEX_DATA_LAYOUT), intent(inout)      :: comp_data
        ...
        if(clean) call cmplx_deleteDatalayout(comp_data)
    ...
    end subroutine cmplx_vector_manipulate_comp

end module cmplxVectordatalayout
!> @}
