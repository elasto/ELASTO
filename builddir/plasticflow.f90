...
!---------------------------------------------------------------------------------
  FUNCTION probabilityL(Lsigma,spec_rank) RESULT(success)
    ...
    !=== INITIALIZE MIN ENERGY BARRIER ===
    Emin=0.002025_WP

    DO k=Lsigma%zmin,Lsigma%zmax
     DO j=Lsigma%ymin,Lsigma%ymax
      DO i=Lsigma%xmin,Lsigma%xmax
        !Lsigma%values(i,j,k) = delta_t * exp( inv *( sigma%values(i,j,k)**2_WP - 1_WP ) ) 
        psr=ran3(seed)
        !CALL random_number(psr)  
        Lsigma%values(i,j,k) = dsqrt( 4_WP*(Emin - (DLOG(1_WP - psr))/(2015_WP) ) )
      ENDDO
     ENDDO
    ENDDO
