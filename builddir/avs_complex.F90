...
!------------------------------------------------------------------------------
subroutine cmplx_GetSizeFromFile(spec_rank,name,mesh_size, success)
!
...
end subroutine cmplx_GetSizeFromFile
...
!> @return .TRUE. if file is successfuly written.
!------------------------------------------------------------------------------
function cmplx_ReadFromFile(spec_rank,name,val) result(success)
!
! Parallel read of a 3D scalar array in one file named 'name'. All the processes write in the same file.
! U, V, W and SC are pointers to 3D arrays defined in "data" module.
!-------------------------------------------------------------------------
    ...
    if (val%nx == nxyz(1) .OR. val%nz == nxyz(2) .OR. val%nz == nxyz(3)) then
        ...
        call MPI_TYPE_CREATE_SUBARRAY(3,gsizes,lsizes,offset,MPI_ORDER_FORTRAN,MPI_COMPLEX_WP,fileview,ierr)
        if (ierr .NE. MPI_SUCCESS) then
            write(6,'(a,i0,a)')"ERROR cmplx_ReadfromFile on process ",spec_rank,": unable to create subarray!"
            return
        end if
        call MPI_TYPE_COMMIT(fileview,ierr)
        if (ierr .NE. MPI_SUCCESS) then
        ...
        call MPI_FILE_SET_VIEW(iunit,disp,MPI_COMPLEX_WP,fileview,"native",MPI_INFO_NULL,ierr)
        if (ierr .NE. MPI_SUCCESS) then
          write(6,'(a,i0,a)')"ERROR cmplx_ReadfromFile on process ",spec_rank,&
                            &": cannot set view on file ["//trim(name)//"]"
          return
        end if
        ! -- Read data --
        call MPI_FILE_READ_ALL(iunit,val%values,lsizes(1)*lsizes(2)*lsizes(3),MPI_COMPLEX_WP,status,ierr)
        if (ierr .NE. MPI_SUCCESS) then
          write(6,'(a,i0,a)')"ERROR success on process ",spec_rank,&
...
end function cmplx_ReadFromFile


end module avs_cmplx
